...
let products = [],cart = [],wishlist = [],electronics=[];
async function fetchAllDate() {
  ...
  electronics = products.filter(prod=>prod.category==='Electronics');
}
...
const init = async () => {
  ...
  addProductHTML(shuffle(electronics));
...
dropDown.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("link")) {
    if (e.target.classList.contains("price-low"))
      ...
      electronics.sort((a, b) => a.title.localeCompare(b.title));
    else if (e.target.classList.contains("alpha-desc"))
      electronics.sort((a, b) => b.title.localeCompare(a.title));
    else 
      electronics = shuffle(electronics);
    addProductHTML(electronics);
