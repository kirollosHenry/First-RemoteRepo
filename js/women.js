...
let products = [],cart = [],wishlist = [],women=[];
async function fetchAllDate() {
  ...
  women = products.filter(prod=>prod.category==='women');
}
...
dropDown.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("link")) {
    if (e.target.classList.contains("price-low"))
      women.sort((a, b) => a.price - b.price);
    else if (e.target.classList.contains("price-high"))
      women.sort((a, b) => b.price - a.price);
      ...
      women.sort((a, b) => b.title.localeCompare(a.title));
    else 
      women = shuffle(women);
    addProductHTML(women);
