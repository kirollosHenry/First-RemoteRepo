...
let products = [],cart = [],wishlist = [],jewellery=[];
async function fetchAllDate() {
  ...
  jewellery = products.filter(prod=>prod.category==='jewellery');
}
...
const init = async () => {
  ...
  addProductHTML(shuffle(jewellery));
...
dropDown.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("link")) {
    if (e.target.classList.contains("price-low"))
      ...
      jewellery.sort((a, b) => a.title.localeCompare(b.title));
    else if (e.target.classList.contains("alpha-desc"))
      jewellery.sort((a, b) => b.title.localeCompare(a.title));
    else 
      jewellery = shuffle(jewellery);
    addProductHTML(jewellery);
